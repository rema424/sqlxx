...
func (di *RepositoryImpl) RunInTx(ctx context.Context, txFn func(context.Context) error) (err, rbErr error) {
	return di.db.RunInTx(ctx, txFn)
}

func (di *RepositoryImpl) CreateUser(ctx context.Context, s Session) (Session, error) {
	q := `INSERT INTO user (email, password) VALUES (:email, :password);`
	res, err := di.db.NamedExec(ctx, q, s.User)
	if err != nil {
		return s, err
	}
...
func (di *RepositoryImpl) CreateSession(ctx context.Context, s Session) (Session, error) {
	q := `INSERT INTO session (id, expire_at, user_id) VALUES (:id, :expire_at, :user.id);`
	_, err := di.db.NamedExec(ctx, q, s)
	return s, err
}
